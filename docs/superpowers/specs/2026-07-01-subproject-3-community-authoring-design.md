...
**(a) In-game editor (main authoring surface).** Full authoring, native Rust core. Editable: per-instrument lane assignment, space-action type, sustain lengths, **feel-targets** (per-note groove nudge), the per-instrument fit/envelope knob (§2), section/structure markers, **lyrics** (see below), and the metadata envelope. Desktop-primary for precision; available in-game.

**Lyrics authoring (vocal part).** MIDI carries no reliable lyric text, so lyrics are **author-entered, not derived from import.** The editor lets the author write the lyrics track for the vocalist: enter **syllable/line text** and **sync each syllable to the vocal part's notes/timing** (snap-to-note assist, with manual nudge). The lyrics track is part of the hashed playable core (§3, SP1's lyrics track) — it drives both the vocalist's on-screen lyrics and the audience view (SP1 §9). An author may leave it empty (instrumental / no-lyrics chart); a partially-lyriced chart is valid.
